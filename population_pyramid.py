...
from bokeh.models import ColumnDataSource, CustomJSTransform, FuncTickFormatter, Select, Slider
...
pyramid = figure(plot_width=800, plot_height=700, toolbar_location=None, y_range=groups,
...
""")
pyramid.sizing_mode = "scale_width"
...
year = Slider(start=1950, end=2010, value=1950, step=5, title="Year")
...
controls = widgetbox(year, location, width=500)
Slider = widgetbox(year, width=600)
curdoc().add_root(row(column(pyramid,Slider), sizing_mode='scale_width'))